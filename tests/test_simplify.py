...
def test_simplify_dependency():
    ...
    gr.update_internal_context(g, {"a": 1, "b": 2, "c": 3, "d": 4})
...
def test_convert_conditional_to_trivial_step_with_default():
    ...
    gr.add_step(g, "c", "identity_recipe", "pre_c")
    gr.add_multiple_conditional(g, "conditional", ["c"], ["v", "default"])
    g["pre_c"] = False
    g["pre_default"] = 1
    g["identity_recipe"] = lambda x: x
    gr.finalize_definition(g)
...
def test_convert_all_conditionals_to_trivial_steps_with_evaluation():
    ...
    gr.add_step(g, "c1", "op_id", "pre_c1")
    gr.add_step(g, "c2", "op_id", "pre_c2")
    gr.add_step(g, "c3", "op_id", "pre_c3")
    g["pre_c1"] = True
    g["pre_c2"] = False
    g["pre_c3"] = False
    g["op_id"] = lambda x: x
