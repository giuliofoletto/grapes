...
def test_simple(expected_sources):
    g = build_graph()
    name = "simple"
    gv = qflow.visualize.get_graphviz_digraph(g)
...
def test_attrs(expected_sources):
    g = build_graph()
    name = "attrs"
    gv = qflow.visualize.get_graphviz_digraph(g, rankdir="LR")
    assert gv.string() == expected_sources[name]
...
def test_no_operations(expected_sources):
    g = build_graph()
    name = "no_operations"
    gv = qflow.visualize.get_graphviz_digraph(g, hide_recipes=True)
    assert gv.string() == expected_sources[name]
